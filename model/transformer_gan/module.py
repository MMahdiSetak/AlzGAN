...
class ImgPatches(nn.Module):
    def __init__(self, input_channel=1, dim=512, patch_size=32):
        super().__init__()
        self.patch_embed = nn.Conv3d(input_channel, dim,
...
class Generator(nn.Module):
    def __init__(self, img_channels=1, image_size=128, patch_size=8, dim=512, depth=4, heads=4,
        ...
        self.positional_embedding = nn.Parameter(torch.zeros(1, self.num_patches_1d ** 3, dim))
        ...
        self.last = nn.Conv3d(dim // 8, img_channels, kernel_size=7, stride=1, padding=3,
...
class ConvolutionBlockD(nn.Module):
    def __init__(self, in_channels, out_channels, stride):
        ...
        self.convolution = nn.Sequential(
            nn.Conv3d(in_channels, out_channels, 4, stride, 1, bias=True, padding_mode="reflect"),
...
class Discriminator(nn.Module):
    def __init__(self, in_channels=1, features=(32, 64, 128, 256), augmentations=None):
        ...
        self.last = nn.Sequential(
            nn.Conv3d(features[3], 1, 4, stride=1, padding=1, padding_mode="reflect"),
